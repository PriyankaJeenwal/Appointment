...
@Service
public class AppointmentServiceImpl<T> implements AppointmentService {
	...
	@Override
	public  AppointmentResponse<Appointment>  getJson(String appointments,String path, List<MultipartFile> files) throws JsonMappingException, JsonProcessingException {
		AppointmentResponse<Appointment> appointmentResponse = new AppointmentResponse<Appointment>();
		...
		for (MultipartFile file : files) {
			...
			try {
				File existFiles = new File("images/" +appointmentobj.getUserId()+"/"+file.getOriginalFilename());
		...
		try {
			if (appointmentobj.getTitle() != null && appointmentobj.getAppointmentDate() != null) {
				...
				Optional<Appointment> existingAppointment = appointmentRepository
						.findAppointmentByTitleAndUserId(appointmentobj.getTitle().toLowerCase(), appointmentobj.getUserId());
